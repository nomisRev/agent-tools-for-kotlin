// Self-contained Gradle init script that registers the `jarSearch` developer utility task on
// every project of a build, without having to apply the `io.github.nomisrev.dev-tools` plugin
// or resolve any external dependency.
//
// The task implementation below is an inlined copy of the plugin source under
// `src/main/kotlin/jarsearch/JarSearchTask.kt`. Keep them in sync.
//
// Usage:
...
//   ./gradlew --init-script jar-search.init.gradle.kts -q jarSearch --dependency group:artifact:version \
//     --kind bytecode --query com.example.FirstType,com.example.SecondType
...
// ---------------------------------------------------------------------------------------------
// Registration: register `jarSearch` on the root project and every subproject. Target a
// specific module with a task path, e.g. `:app:jarSearch`.
// ---------------------------------------------------------------------------------------------

gradle.allprojects {
  val project = this
  if (tasks.findByName("jarSearch") == null) {
    tasks.register<JarSearch.JarSearchTask>("jarSearch") {
...
data class ModuleCoordinate(val group: String, val name: String, val version: String?) {
  val notation: String
    get() = if (version == null) "$group:$name" else "$group:$name:$version"
}

data class SelectedJars(
...
enum class SearchKind(val searchesMembers: Boolean) {
...
data class SearchOutput(val sections: List<ResultSection>) {
  val hasResults: Boolean = sections.any { it.total > 0 }
}

data class ResultSection(val title: String, val total: Int, val lines: List<String>)
...
fun searchJars(jars: List<File>, options: SearchOptions): SearchOutput {
...
fun packageSection(jarIndexes: List<JarIndex>, options: SearchOptions): ResultSection {
  val matches =
    jarIndexes
...
fun typeSection(jarIndexes: List<JarIndex>, options: SearchOptions): ResultSection {
  val exactTypes = exactTypeMatches(jarIndexes, options.query)
  val matches =
...
fun exactTypeMatches(jarIndexes: List<JarIndex>, query: String?): List<ClassSearchResult> {
...
fun memberSection(jarIndexes: List<JarIndex>, options: SearchOptions): ResultSection {
...
data class JarIndex(val jar: File, val classes: List<ClassEntry>, val packages: List<String>) {
  companion object {
...
data class ClassEntry(val internalName: String) {
...
data class ClassSearchResult(val entry: ClassEntry, val jar: File) {
...
data class ClassInfo(val methods: List<MethodInfo>)
...
fun looksCompilerGenerated(name: String): Boolean =
...
data class ParsedType(val type: String, val nextIndex: Int)

fun String.simplifyJavaLang(): String =
  when (this) {
...
fun DataInputStream.skipFully(bytes: Int) {
  var remaining = bytes
  while (remaining > 0) {
...
fun String.normalizeTypeQuery(): String = replace('$', '.')
